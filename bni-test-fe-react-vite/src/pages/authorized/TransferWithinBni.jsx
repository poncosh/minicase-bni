...
export const TransferWithinBni = () => {
  const { authState, onTransactionInternal } = useAuth();
  ...
  useEffect(() => {
    document.title = "MiniCase BNI | Transfer BNI";
  }, []);
  ...
  const submitTopUp = () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const item = await onTransactionInternal(data);
        if (item && item.error) {
          return Swal.fire({
            icon: "error",
            title: `Top Up sebesar ${new Intl.NumberFormat("id-ID", {
